...
func TestParseReceivePackAcceptsCommandsAndPreservesRawBytes(t *testing.T) {
	raw := joinPackets(
	...
	if len(result.Updates) != 2 || result.Updates[0].Ref != "refs/heads/feature" || result.Updates[1].Ref != "refs/tags/v1" {
...
func TestParseReceivePackAcceptsShallowDeclarationsBeforeOrdinaryCommands(t *testing.T) {
	...
	if len(result.Updates) != 1 || result.Updates[0].Ref != "refs/heads/feature" || !bytes.Equal(result.Raw, raw) {
...
func TestParseReceivePackRejectsMainUnderDefaultPolicy(t *testing.T) {
	...
	result, err := ParseReceivePack(bytes.NewReader(raw), ReceiveOptions{
		...
		Policy:         config.DefaultPushPolicy(),
	})
...
func TestParseReceivePackAcceptsShallowPushCertificateAndAdvertisedOptions(t *testing.T) {
	...
	if !bytes.Equal(result.Raw, raw) || len(result.Updates) != 1 || result.Updates[0].Ref != "refs/heads/feature" {
		t.Fatalf("result = %#v", result)
	}
}
...
func TestParseReceivePackAcceptsSHA256DeletesAndForceShapedUpdates(t *testing.T) {
	...
	if len(result.Updates) != 3 || !bytes.Equal(result.Raw, raw) {
		t.Fatalf("result = %#v", result)
	}
}
...
func TestParseReceivePackAppliesPolicyToSignedCertificateCommands(t *testing.T) {
	...
	if result.Raw != nil {
		t.Fatalf("Raw = %q, want nil", result.Raw)
	}
}
