...
func TestParseAdvertisedCapabilitiesPreservesAdvertisement(t *testing.T) {
	raw := joinPackets(
	...
	result, err := ParseAdvertisedCapabilities(fragmented(raw, 3), 4096)
	if err != nil {
...
func TestParseAdvertisedCapabilitiesRejectsMalformedFirstReference(t *testing.T) {
	for name, raw := range map[string][]byte{
		...
		t.Run(name, func(t *testing.T) {
			...
			result, err := ParseAdvertisedCapabilities(bytes.NewReader(raw), limit)
			if err == nil {
