...
func TestParseReceivePackRequiresAdvertisedDeleteRefsForDeletes(t *testing.T) {
	for name, testCase := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			unadvertised := ReceiveOptions{MaxBytes: 4096, AdvertisedCaps: testCase.advertised}
			...
			advertised := ReceiveOptions{MaxBytes: 4096, AdvertisedCaps: make(Capabilities, len(testCase.advertised)+1)}
			...
			if err != nil || !bytes.Equal(result.Raw, testCase.raw) || len(result.Updates) != 1 {
