...
func TestParseReceivePackAcceptsCapturedGitV250ShallowPayload(t *testing.T) {
	...
	if err != nil || !bytes.Equal(result.Raw, raw) || len(result.Updates) != 1 {
...
func TestParseAdvertisedCapabilitiesAcceptsHaveOnlyAsAdvertisementRef(t *testing.T) {
	for name, raw := range map[string][]byte{
		...
		t.Run(name, func(t *testing.T) {
			result, err := ParseAdvertisedCapabilities(fragmented(raw, 1), 4096)
...
func TestParseAdvertisedCapabilitiesRejectsHaveNearVariants(t *testing.T) {
	for name, raw := range map[string][]byte{
		...
		t.Run(name, func(t *testing.T) {
			result, err := ParseAdvertisedCapabilities(bytes.NewReader(raw), 4096)
			if err == nil {
