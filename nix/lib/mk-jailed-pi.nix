...
in
{
  mkJailedPi =
    ...
    let
      ...
      githubBrokerCfg = githubBrokerLib.normalize githubBroker;
      githubBrokerEnabled = builtins.deepSeq githubBrokerCfg githubBrokerCfg.enable;
      githubBrokerPackage = githubBrokerServerPackage;
      githubBrokerClientPackage = self'.packages."jailed-github-broker".client;
      githubBrokerConfigFile = githubBrokerLib.mkConfigFile githubBrokerCfg;
      githubCredentialEnvironmentNames = [
        "GH_TOKEN"
        "GITHUB_TOKEN"
        "GH_ENTERPRISE_TOKEN"
      ...
      piRuntime = pkgs.writeShellApplication {
        name = "pi";
        runtimeInputs = lib.optional githubBrokerEnabled githubBrokerClientPackage ++ [ pkgs.coreutils ];
        text = ''
          set -euo pipefail
      ...
      sandbox = jail "${name}-sandbox" piRuntime (
        ...
        ++ lib.optionals githubBrokerEnabled [
          (add-runtime ''
            if [ -z "''${${githubBrokerLib.hostSocketEnvironment}:-}" ] \
              || [ ! -S "${"$"}${githubBrokerLib.hostSocketEnvironment}" ]; then
              echo "jailed GitHub broker: verified host socket is unavailable" >&2
              exit 1
            fi
            RUNTIME_ARGS+=(
              --dir /run
              --dir /run/jailed-github-broker
              --bind "${"$"}${githubBrokerLib.hostSocketEnvironment}" ${lib.escapeShellArg githubBrokerLib.stableSocketPath}
            )
            unset ${githubBrokerLib.hostSocketEnvironment}
          '')
          (set-env "JAILED_GITHUB_BROKER_SOCKET" githubBrokerLib.stableSocketPath)
          (set-env "JAILED_GITHUB_BROKER_REPOSITORY" githubBrokerCfg.repository)
          (set-env "GIT_SSH_COMMAND" "${githubBrokerClientPackage}/bin/jailed-git-ssh")
        ]
        ...
        ++ [
          (add-pkg-deps (
            ...
            ++ containerPkgs
            ++ extraPkgs
            ++ lib.optional githubBrokerEnabled githubBrokerClientPackage
          ))
        ]
      );
    in
    assert lib.assertMsg (
      !githubBrokerEnabled || lib.intersectLists githubCredentialEnvironmentNames apiKeyNames == [ ]
    ) "mkJailedPi githubBroker forbids forwarding GitHub tokens or SSH_AUTH_SOCK into the jail";
    pkgs.writeShellApplication {
      ...
      ++ lib.optionals githubBrokerEnabled githubBrokerLib.lifecycleRuntimeInputs;
      text = ''
        ...
        ''}
        ${
          if githubBrokerEnabled then
            githubBrokerLib.mkLifecycleScript {
              brokerPackage = githubBrokerPackage;
              configFile = githubBrokerConfigFile;
              jailExecutable = lib.getExe sandbox;
            }
          else
