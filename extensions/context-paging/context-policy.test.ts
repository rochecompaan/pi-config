...
const selected = (messages: readonly object[], modelContextWindow = 100_000, rawHistoryItems?: HistoryItem[]) => selectContext({
...
test("accounts resident inputs and canonical messages with Pi estimates", () => {
	...
	assert.equal(selectContext({ messages: [], systemPrompt: "resident", activeTools: [], modelContextWindow: 32_000 }).budgetTokens, 32_000);
});
...
test("adds a stable raw history ID to a notice for a matched evicted message", () => {
	...
	const selection = selected(messages, 100_000, rawHistoryItems);
...
test("keeps a protected unread trailing parallel tool exchange for one follow-up call", () => {
	...
	assert.ok(selection.estimatedTokens > 64_000);
...
test("reports invalid model windows and resident or active request overflows as estimates", () => {
	for (const modelContextWindow of [undefined, 0, Number.NaN, -1]) {
		assert.throws(
			() => selectContext({ messages: [], systemPrompt: "resident", activeTools: [], modelContextWindow }),
			(error: unknown) => error instanceof ContextSelectionError
				&& error.code === "INVALID_MODEL_CONTEXT" && /estimate/i.test(error.message),
		);
	}
	assert.throws(
		() => selectContext({ messages: [], systemPrompt: "x".repeat(300_000), activeTools: [], modelContextWindow: 64_000 }),
		(error: unknown) => error instanceof ContextSelectionError
			&& error.code === "RESIDENT_INPUT_TOO_LARGE" && /estimate/i.test(error.message),
	);
	assert.throws(
		() => selectContext({ messages: [user("x".repeat(300_000))] as any, systemPrompt: "resident", activeTools: [], modelContextWindow: 64_000 }),
		(error: unknown) => error instanceof ContextSelectionError
