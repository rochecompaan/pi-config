...
export type HistoryNavigatorErrorCode = "INVALID_HISTORY_NAVIGATOR_INPUT" | "UNKNOWN_HISTORY_ANCHOR" | "UNKNOWN_HISTORY_ID";

export class HistoryNavigatorError extends Error {
	readonly code: HistoryNavigatorErrorCode;

	constructor(code: HistoryNavigatorErrorCode, message = code) {
		super(message);
		this.code = code;
...
function itemText(item: HistoryItem): string {
	...
	const toolCalls = Array.isArray(item.assistantMessage.content)
		? item.assistantMessage.content.flatMap((block) => {
...
function previewFor(item: HistoryItem): string {
	...
	const content = Array.isArray(item.assistantMessage.content)
		? item.assistantMessage.content.map((block) =>
...
function buildReference(item: HistoryItem, index: number, items: readonly HistoryItem[]): HistoryReference {
	...
	return {
		...
		previousHistoryId: items[index - 1]?.id,
		nextHistoryId: items[index + 1]?.id,
...
export class HistoryNavigator {
	...
	search(input: HistorySearchInput): HistoryReference[] {
		...
		return this.items
			...
			.sort((left, right) => right.score - left.score || left.item.sequence - right.item.sequence)
			.slice(0, limit)
